...
import {CHANGE_ACCOUNT_PW} from '../../store/user/mutation-types'
...
export const userMixin = {
  ...
  methods: {
    _changeAcoPw() {
      store.commit(CHANGE_ACCOUNT_PW, !this.changeAccountPw)
    },
    _getUserInfo(callback) {
      const openid = openidUtils.get()
      if (!openid) return
      fetchUserInfo({loginName: this.loginAccount.loginName}).then(res => {
        res.code = res['STR_USER_CODE']
