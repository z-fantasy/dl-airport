 ...
 * 将文件地址拼接为标签链接
 */
export function joinHtml(prefix, url) {
    if (!url) return ''
    if (/\.bmp|\.jpg|\.png|\.tif|\.gif|\.svg/.test(url)) {
        return `<img src="${prefix + url}" class="fileImg"/>`
    } else if (/\.mp4/.test(url)) {
        let videoUrl = prefix + url
        // if (!IS_PRODUCTION) {
        //     videoUrl = videoUrl.replace(/\/resource/, '/CRM31_DaLianJiChang/resource')
        // }
        return `<video src="${videoUrl}" controls="controls">您的浏览器不支持 video 标签。</video>`
    } else {
        return `<a href="${prefix + url}" class="fileUrl"/>${url.replace('/', '')}</a>`
    }
}
/**
 * 取出html里面的src地址
 */
export function getHtmlSrc(htmlStr, tag = 'video', attr = 'src') {
    let str = htmlStr
    let imgReg = new RegExp(`<${tag}.*?(?:>|\\/>)`, 'gi')
    let srcReg = new RegExp(`${attr}=[\\\'\\"]?([^\\\'\\"]*)[\\\'\\"]?`, 'i') // /src=[\'\"]?([^\'\"]*)[\'\"]?/i
    let arr = str.match(imgReg);  // arr 为包含所有img标签的数组
    let tagArr = []
    if  (arr) {
        for (let i = 0; i < arr.length; i++) {
            const srcArr = arr[i].match(srcReg)
            if (srcArr && srcArr.length >= 2) {
                tagArr = [...tagArr, srcArr[1]]
            }
        }
    }
    return tagArr
