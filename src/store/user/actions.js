import {saveUser, loadUser, clearUser, loadAccount, saveLoginAccount, clearAct} from '../../common/cache'
...
// 登录账号相关
export const saveAccount = function ({commit}, accounts) {
    if (accounts) {
        const oldAccont = loadAccount()
        let nowAccont = oldAccont ? oldAccont : accounts
      if (oldAccont) {
          nowAccont = Object.assign({}, oldAccont, accounts)
      }
        saveLoginAccount(nowAccont)
        commit(types.LOGIN_ACCOUNT, nowAccont)
    }
}
export const clearAccount = function ({commit}) {
    clearAct()
    commit(types.LOGIN_ACCOUNT, false)
}
